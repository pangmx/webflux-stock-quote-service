...
public class QuoteGeneratorServiceImpl implements QuoteGeneratorService {
    ...
    public Flux<Quote> fetchQuoteStream(Duration period) {
        ...
        // we are using Flux.generate to create quotes,
        // iterating on each stock starting at index 0
        return Flux.generate(() -> 0,
                (BiFunction<Integer, SynchronousSink<Quote>, Integer>) (index, quoteSynchronousSink) -> {
                    Quote updatedQuote = updateQuote(this.prices.get(index));
                    quoteSynchronousSink.next(updatedQuote);
                ...
                // zipWith returns a Flux tuple of Quote and Period,
                // hence we need to specially map the result to return just Flux Quote.
                .map(t -> t.getT1())
    ...
    private Quote updateQuote(Quote quote){
        // get a random price change amount as 5% of the next random double
        BigDecimal priceChange = quote.getPrice()
                .multiply(new BigDecimal(0.05 * this.random.nextDouble(), this.mathContext));

        // return a new Quote adding the new random price change amount
